...
export function apply(ctx) {
  ...
  registerPrismFlowTool(ctx, defineTool({
    ...
    description: 'Default all-sources path. Create one immutable low-token AI-only Selection Artifact across every persisted source, using the deployment-owned item and token budgets: complete relevance preparation, bounded ambiguity review, event deduplication, diverse ranking, and bounded verbatim material packing. This tool cannot narrow to one source or lower item/token budgets. A window below 48 hours or category/topic filter requires explicit user approval.',
