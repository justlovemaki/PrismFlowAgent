// Generated from src/core/content/AIRelevance.ts by integrations/dsh/scripts/sync-shared.mjs.
...
const STRONG_PHRASES = Object.entries(TOPIC_PHRASES)
    .flatMap(([topic, phrases]) => phrases.map(phrase => ({ topic: topic, phrase })));
...
function metadataSummary(metadata) {
    if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata))
        return '';
    return rawString(metadata.ai_summary);
}
export function rawAIRelevanceFields(record) {
    const item = record?.item;
    if (!item || typeof item !== 'object' || Array.isArray(item))
        throw new Error('Relevance record item is invalid');
    return {
...
export function aiRelevanceInputChars(record) {
...
export function aiRelevanceContentHash(record, maxInputChars = 2_000_000) {
    ...
    if (inputChars > maxInputChars)
        throw new Error('AI relevance hash input exceeds the configured per-record limit');
...
function combinedMatcher(phrases) {
...
function validLatinBoundary(text, index, length, phrase) {
    if (hasHan(phrase) || !/[a-z]/iu.test(phrase))
        return true;
    const before = index > 0 ? text[index - 1] : '';
    const after = index + length < text.length ? text[index + length] : '';
    return !/[A-Za-z0-9_]/u.test(before) && !/[A-Za-z0-9_]/u.test(after);
}
function matches(text, matcher, allowed) {
    matcher.lastIndex = 0;
    const result = [];
    for (const match of text.matchAll(matcher)) {
        const phrase = normalizedPhrase(match[0]);
        if (!allowed.has(phrase) || !validLatinBoundary(text, match.index ?? 0, match[0].length, phrase))
            continue;
        result.push({ index: match.index ?? 0, phrase });
        if (result.length >= MAX_MATCHES_PER_FIELD)
            break;
    }
    return result;
}
...
function boundedExcerpt(text, index, maxChars) {
    ...
    if (end === text.length && end - start < maxChars)
        start = Math.max(0, end - maxChars);
    const value = text.slice(start, end).replace(/[\u0000-\u001f\u007f]/gu, ' ').trim();
    return `${start > 0 ? '…' : ''}${value}${end < text.length ? '…' : ''}`.slice(0, maxChars);
}
function sampledText(value, maxChars) {
    if (value.length <= maxChars)
        return { text: value, consumedChars: value.length, truncated: false };
...
export function assessAIRelevance(record, options = {}) {
    ...
    if (!Number.isInteger(maxScanChars) || maxScanChars < 1_024 || maxScanChars > 2_000_000)
        throw new Error('maxScanChars is invalid');
    if (!Number.isInteger(maxEvidence) || maxEvidence < 1 || maxEvidence > 32)
        throw new Error('maxEvidence is invalid');
    if (!Number.isInteger(maxEvidenceChars) || maxEvidenceChars < 40 || maxEvidenceChars > 500)
        throw new Error('maxEvidenceChars is invalid');
    ...
    for (const field of fields) {
        ...
        for (const match of found) {
            ...
            strongCount += 1;
            if (field.field === 'title' || field.field === 'ai_summary')
                titleOrSummaryStrong = true;
    ...
    if (strongCount === 0) {
        for (const field of fields) {
            ...
            if (found.length > 0 && evidence.length < maxEvidence)
                evidence.push({
    ...
    const reasonCodes = [];
    let verdict;
    if (titleOrSummaryStrong) {
        verdict = 'matched-ai';
        reasonCodes.push('strong-title-or-summary');
    }
    else if (topics.size >= 2) {
        verdict = 'matched-ai';
        reasonCodes.push('multiple-ai-topic-families');
    }
    else if (strongCount >= 2) {
        verdict = 'matched-ai';
        reasonCodes.push('repeated-strong-ai-evidence');
    }
    else if (strongCount === 1) {
        verdict = 'ambiguous';
        reasonCodes.push('single-strong-body-signal');
    }
    else if (weakCount > 0) {
        verdict = 'ambiguous';
        reasonCodes.push('ambiguous-ai-terms-only');
    }
    else if (truncated) {
        verdict = 'ambiguous';
        reasonCodes.push('scan-truncated-without-signal');
    }
    else {
        verdict = 'unmatched';
        reasonCodes.push('no-recognized-ai-evidence');
    }
    if (truncated && !reasonCodes.includes('scan-truncated-without-signal'))
        reasonCodes.push('scan-truncated');
...
export function buildAIRelevanceCard(record, assessment, maxChars = 2_000) {
    if (!Number.isInteger(maxChars) || maxChars < 512 || maxChars > 8_000)
    ...
    if (JSON.stringify(card).length <= maxChars)
        return card;
    const compact = {
    ...
    while (JSON.stringify(compact).length > maxChars && compact.evidence.length > 0)
        compact.evidence.pop();
    while (JSON.stringify(compact).length > maxChars && compact.topics.length > 0)
        compact.topics.pop();
    while (JSON.stringify(compact).length > maxChars && compact.reasonCodes.length > 0)
        compact.reasonCodes.pop();
    if (JSON.stringify(compact).length > maxChars)
        compact.title = compact.title.slice(0, 40);
    if (JSON.stringify(compact).length > maxChars)
        throw new Error('maxChars is too small for a compact relevance card');
    return compact;
}
