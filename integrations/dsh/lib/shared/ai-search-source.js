// Generated from src/core/sources/AISearchSource.ts by integrations/dsh/scripts/sync-shared.mjs.
...
export function buildAISearchPrompt(keyword, limit) {
    if (!keyword.trim())
        throw new Error('AI Search keyword is required');
...
export function parseAISearchItems(content) {
    if (!content.trim())
        return [];
...
export function normalizeAISearchItems(items, options) {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
        ...
        try {
            if (!item || typeof item !== 'object' || Array.isArray(item))
                continue;
            if (typeof item.title !== 'string' || !item.title.trim() || typeof item.url !== 'string' || typeof item.content !== 'string' || !item.content.trim())
                continue;
            let url;
            try {
