// Generated from src/core/publishing/R2Publisher.ts by integrations/dsh/scripts/sync-shared.mjs.
...
export function normalizeR2AccountId(value) {
    const accountId = requirePlainText(value, 'R2 accountId', 32).toLowerCase();
    if (!/^[a-f0-9]{32}$/.test(accountId))
        throw new Error('R2 accountId must contain exactly 32 hexadecimal characters');
    return accountId;
}
export function validateR2BucketName(value) {
...
export function validateR2PathPrefix(value) {
    if (value === '')
        return '';
...
export function validateR2ObjectKey(value) {
    ...
    if (Buffer.byteLength(value, 'utf8') > 1_024)
        throw new Error('R2 publication object key exceeds 1024 UTF-8 bytes');
    return value;
}
export function buildR2ObjectKey(pathPrefix, fileName) {
...
export function buildR2ApiEndpoint(accountId) {
    return `https://${normalizeR2AccountId(accountId)}.r2.cloudflarestorage.com`;
}
export function buildR2ObjectUrl(accountId, bucket, key) {
...
export function normalizeR2PublicUrlPrefix(value) {
    if (value === '')
        return undefined;
    let url;
    try {
...
export function buildR2PublicObjectUrl(prefix, key) {
