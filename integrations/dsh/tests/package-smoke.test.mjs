...
test('packed package installs standalone and its publisher-profile bin exports an isolated Profile', { timeout: 180_000 }, async t => {
  ...
  const installedSharp = JSON.parse(readFileSync(join(root, 'node_modules', 'sharp', 'package.json'), 'utf8'))
  assert.equal(installedManifest.dependencies.sharp, '0.35.4', 'PrismFlow must share the tested DSH Sharp runtime on Windows')
  assert.equal(installedSharp.version, '0.35.4')
  const sharpSmoke = join(root, 'sharp-smoke.cjs')
  writeFileSync(sharpSmoke, "require('sharp')({ create: { width: 1, height: 1, channels: 4, background: '#000' } }).png().toBuffer().then(() => console.log('sharp-ok'))\n")
  run(process.execPath, [sharpSmoke], { cwd: root })
  ...
  assert.equal(existsSync(join(installed, 'manual-import', 'plugins', 'prismflow-personal-cover-generation.zip')), true)
