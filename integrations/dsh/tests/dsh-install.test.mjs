...
test('main installer writes runtime, storage, and Dashboard in one Profile patch update', async t => {
  ...
  assert.equal((await readdir(join(manualRoot, 'plugins'))).filter(name => name.endsWith('.zip')).length, 7)
...
test('published package exposes the explicit installer without a postinstall hook', async () => {
  ...
  assert.equal(packageJson.bin['prismflow-dsh-update'], './scripts/update.mjs')
  ...
  assert.match(result.stderr, /prismflow-dsh-update/u)
  const updaterHelp = spawnSync(process.execPath, [fileURLToPath(new URL('../scripts/update.mjs', import.meta.url)), '--help'], { encoding: 'utf8' })
  assert.equal(updaterHelp.status, 0, updaterHelp.stderr)
  assert.match(updaterHelp.stderr, /prismflow-dsh-update --package/u)
  assert.match(updaterHelp.stderr, /restarts and health-checks DSH Web/u)
  const source = await readFile(new URL('../scripts/install.mjs', import.meta.url), 'utf8')
  const updaterSource = await readFile(new URL('../scripts/update.mjs', import.meta.url), 'utf8')
  assert.match(source, /starting verified automatic migration/u)
  assert.match(source, /listProcProcesses/u)
  assert.match(source, /process inspection must be unconditional/u)
  assert.ok(source.lastIndexOf('assertDshIsOffline(options)') < source.indexOf('const releaseLocks = await acquireInstallationLocks'))
  assert.ok(source.lastIndexOf('assertDshIsOffline(options)') < source.indexOf("run('pnpm'"))
  ...
  assert.match(updaterSource, /await stopDshProcesses\(before\)/u)
  assert.ok(updaterSource.indexOf('await stopDshProcesses(before)') < updaterSource.indexOf("run('pnpm', ['dlx'"))
  assert.ok(updaterSource.indexOf("run('pnpm', ['dlx'") < updaterSource.indexOf("run(process.execPath, [installer"))
  assert.ok(updaterSource.indexOf("run(process.execPath, [installer") < updaterSource.lastIndexOf('startDshWeb(options, port)'))
  assert.match(updaterSource, /waitForHealth\(port\)/u)
  assert.match(updaterSource, /if \(updateError && restartError\)/u)
  const escapedVersion = packageJson.version.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  assert.match(readme, new RegExp(`prismflow-dsh-update --package .*prismflow-dsh-${escapedVersion}\\.tgz`, 'u'))
