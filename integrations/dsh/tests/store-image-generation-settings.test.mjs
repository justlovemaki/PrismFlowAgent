...
test('persists CAS-versioned image settings and validates safe OpenAI-compatible endpoints', async () => {
  ...
  const initial = value.service.get(); assert.equal(initial.version, 1); assert.equal(initial.imageModel, 'gpt-image-1')
  const next = await value.service.update({ imageApiUrl: 'https://images.example/v1/chat/completions', imageApiProtocol: 'chat-completions', imageModel: 'image-pro', imageSize: '1536x1024', avifQuality: 75, avifEffort: 6 }, { version: initial.version, sha256: initial.sha256 })
  assert.equal(next.version, 2); assert.equal(value.service.runtime().imageModel, 'image-pro'); assert.equal(value.history.map.size, 2)
