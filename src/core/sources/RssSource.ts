...
export interface NormalizeRssOptions {
  feedId: string;
  name: string;
  category: string;
  now?: Date;
...
function cleanText(value: unknown): string {
  return asString(value)
    ...
    .replace(/```css[\s\S]*?```/gi, ' ')
    .replace(/<img[^>]*src="([^"]*)"[^>]*alt="([^"]*)"[^>]*>/gi, (_match, src: string, alt: string) => (
      alt ? `[图片: ${alt} ${src}]` : `[图片: ${src}]`
    ))
...
function stableItemId(feedId: string, item: ParsedRssItem, index: number): string {
...
export function validateRssFeedDefinition(feed: RssFeedDefinition): void {
  if (!feed.id.trim()) throw new Error('RSS feed id is required');
  if (!feed.name.trim()) throw new Error(`RSS feed ${feed.id} name is required`);
...
export async function fetchParsedRssFeed(
  ...
  const parsed = await parser.parseString(await response.text()) as unknown as {
    title?: unknown;
    items?: ParsedRssItem[];
  };
...
export function normalizeParsedRssFeed(
  rawFeed: ParsedRssFeed,
  options: NormalizeRssOptions,
): NormalizedRssItem[] {
  const now = options.now ?? new Date();
  const nowIso = now.toISOString();

  const results: NormalizedRssItem[] = [];
  const items = Array.isArray(rawFeed?.items) ? rawFeed.items : [];
  for (let index = 0; index < items.length; index += 1) {
    const item = items[index];
    try {
      if (!item || typeof item !== 'object' || Array.isArray(item) || typeof item.title !== 'string' || !item.title.trim()) continue;
