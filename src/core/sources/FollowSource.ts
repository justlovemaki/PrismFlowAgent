...
export interface FollowEntryData {
  id?: unknown;
  title?: unknown;
  url?: unknown;
  content?: unknown;
...
export interface FollowEntry {
  entries?: FollowEntryData;
  feeds?: FollowFeedData;
}

export interface FollowRawData {
  data: FollowEntry[];
}

export interface NormalizedFollowItem {
  id: string;
  title: string;
  url: string;
  description: string;
...
export interface NormalizeFollowOptions extends FollowRequestOptions {
  now?: Date;
  limit?: number;
}
...
function headers(options: FollowRequestOptions, withJson: boolean): Record<string, string> {
  return {
...
function sleep(milliseconds: number, implementation?: (milliseconds: number) => Promise<void>): Promise<void> {
...
function parseEntries(value: unknown): FollowEntry[] {
  if (!value || typeof value !== 'object') return [];
  const data = (value as { data?: unknown }).data;
  return Array.isArray(data) ? data as FollowEntry[] : [];
}

export function validateFollowSourceDefinition(definition: FollowSourceDefinition): void {
  if (!definition.id.trim()) throw new Error('Follow source id is required');
  if (!definition.name.trim()) throw new Error(`Follow source ${definition.id} name is required`);
...
export async function fetchFollowEntries(
  definition: FollowSourceDefinition,
  options: FollowRequestOptions = {},
): Promise<FollowRawData> {
  validateFollowSourceDefinition(definition);
  const fetchImpl = options.fetchImpl ?? fetch;
  const allEntries: FollowEntry[] = [];
  let publishedAfter: string | undefined;

  for (let page = 0; page < definition.fetchPages; page += 1) {
    const body: Record<string, unknown> = { view: definition.view };
    if (definition.view === 1) body.withContent = true;
    if (definition.listId) body.listId = definition.listId;
    if (definition.feedId) body.feedId = definition.feedId;
...
export async function normalizeFollowEntries(
  rawData: FollowRawData,
  definition: FollowSourceDefinition,
  options: NormalizeFollowOptions = {},
): Promise<NormalizedFollowItem[]> {
  validateFollowSourceDefinition(definition);
  ...
  const results: NormalizedFollowItem[] = [];
  ...
  for (const item of items) {
    ...
    try {
      ...
      if (!description && !/<(?:img|video)\b/i.test(content)) continue;
