...
function hasInvalidUnicode(value: unknown): boolean {
...
export function contentStoreId(sourceId: string, externalId: string): string {
...
export function prepareStoredContentRecord(
  sourceId: string,
  input: unknown,
  existing?: StoredContentRecord,
  now: Date = new Date(),
): StoredContentRecord {
...
export function countStoredContent(
  records: Iterable<StoredContentRecord>,
  query: Omit<ContentQuery, 'limit' | 'offset'> = {},
): number {
  const needle = query.search?.trim().toLocaleLowerCase();
  let count = 0;
  for (const record of records) {
    if (matchesStoredContent(record, query, needle)) count += 1;
  }
  return count;
}

export function queryStoredContent(
  records: Iterable<StoredContentRecord>,
  query: ContentQuery = {},
): StoredContentRecord[] {
