...
export interface SelectionCandidate {
  record: RelevanceRecordLike;
  contentHash: string;
  assessment: AIRelevanceAssessment;
  relevanceOrigin: RelevanceOrigin;
  effectiveTimestamp: number;
  editorial?: AIEditorialResult;
}

export interface EventCluster {
  clusterId: string;
  members: SelectionCandidate[];
...
export function aiSelectionContentHash(record: RelevanceRecordLike, maxChars = 12_000_000): string {
  if (!Number.isInteger(maxChars) || maxChars < 1_024 || maxChars > 20_000_000) throw new Error('Selection content hash limit is invalid');
  const item = record.item as Record<string, unknown>;
  const metadata = item.metadata && typeof item.metadata === 'object' && !Array.isArray(item.metadata)
    ? item.metadata as Record<string, unknown> : {};
  const fields: Array<[string, string]> = [
    ['sourceId', record.sourceId],
    ...(['title', 'url', 'description', 'content', 'source', 'author', 'published_date', 'category'] as const)
      .map(field => [field, typeof item[field] === 'string' ? item[field] as string : ''] as [string, string]),
  ...
  for (const [field, value] of fields) {
    chars += value.length;
    if (chars > maxChars) throw new Error('Selection content hash input exceeds the configured limit');
    hash.update(`${field.length}:`).update(field, 'utf8').update(`${Buffer.byteLength(value, 'utf8')}:`).update(value, 'utf8');
  }
  return hash.digest('hex');
}

export function canonicalSelectionUrl(value: unknown): string {
  ...
  try {
    ...
    for (const name of [...url.searchParams.keys()]) {
      if (/^(utm_|fbclid|gclid)/iu.test(name)) url.searchParams.delete(name);
    }
...
export function normalizedTitleTokens(value: unknown): string[] {
  const title = clean(value, 2_000).toLocaleLowerCase('en-US');
  const tokens = new Set<string>();
  for (const match of title.matchAll(/[a-z0-9][a-z0-9._+-]*/gu)) {
    if (match[0].length > 1 && !STOPWORDS.has(match[0])) tokens.add(match[0]);
  }
  const hanRuns = title.match(/[\u3400-\u9fff]+/gu) ?? [];
  for (const run of hanRuns) {
    if (run.length === 1 && !STOPWORDS.has(run)) tokens.add(run);
...
export function titleSimHash(tokens: string[]): bigint {
  const weights = Array<number>(64).fill(0);
  for (const token of tokens) {
    const hash = hash64(token);
    for (let bit = 0; bit < 64; bit += 1) weights[bit] += ((hash >> BigInt(bit)) & 1n) === 1n ? 1 : -1;
  }
  let result = 0n;
...
export function tokenJaccard(left: string[], right: string[]): number {
  ...
  let intersection = 0;
  for (const token of l) if (r.has(token)) intersection += 1;
  return intersection / (l.size + r.size - intersection);
}

function topicCompatible(left: SelectionCandidate, right: SelectionCandidate): boolean {
  if (left.assessment.topics.length === 0 || right.assessment.topics.length === 0) return true;
...
class DisjointSet {
  ...
  find(value: number): number { let current = value; while (this.parent[current] !== current) current = this.parent[current]; while (this.parent[value] !== value) { const next = this.parent[value]; this.parent[value] = current; value = next; } return current; }
  union(left: number, right: number): void { const l = this.find(left); const r = this.find(right); if (l !== r) this.parent[Math.max(l, r)] = Math.min(l, r); }
}

function candidateBodyChars(candidate: SelectionCandidate): number {
  const item = candidate.record.item;
  return (typeof item.description === 'string' ? item.description.length : 0)
    + (typeof item.content === 'string' ? item.content.length : 0);
}

function candidateAIScore(candidate: SelectionCandidate): number {
  return Number.isInteger(candidate.editorial?.aiScore) ? candidate.editorial!.aiScore : 0;
}

function representativeTuple(candidate: SelectionCandidate, sourceCount: number): [number, number, number, number, string] {
...
function compareTupleDescending(left: [number, number, number, number, string], right: [number, number, number, number, string]): number {
  for (let index = 0; index < 4; index += 1) { const delta = right[index] as number - (left[index] as number); if (delta !== 0) return delta; }
  return (left[4] as string).localeCompare(right[4] as string);
}

export function clusterAIEvents(candidates: SelectionCandidate[], options: ClusteringOptions = {}): EventCluster[] {
  ...
  if (!Number.isInteger(maxBucketSize) || maxBucketSize < 2 || maxBucketSize > 1_000) throw new Error('maxBucketSize is invalid');
  if (!Number.isInteger(maxPairComparisons) || maxPairComparisons < 1 || maxPairComparisons > 2_000_000) throw new Error('maxPairComparisons is invalid');
  if (!(minJaccard >= 0.5 && minJaccard <= 1) || !Number.isInteger(maxHamming) || maxHamming < 0 || maxHamming > 16) throw new Error('Clustering thresholds are invalid');
  const sorted = [...candidates].sort((a, b) => a.record.storeId.localeCompare(b.record.storeId));
  if (new Set(sorted.map(item => item.record.storeId)).size !== sorted.length) throw new Error('Selection candidates contain duplicate store ids');
  ...
  for (let index = 0; index < sorted.length; index += 1) {
    for (const key of [metadata[index].url && `u:${metadata[index].url}`, metadata[index].title && `t:${metadata[index].title}`].filter(Boolean) as string[]) {
      const prior = exact.get(key); if (prior === undefined) exact.set(key, index); else dsu.union(prior, index);
    }
  }
  const tokenFrequency = new Map<string, number>();
  for (const item of metadata) for (const token of item.tokens) tokenFrequency.set(token, (tokenFrequency.get(token) ?? 0) + 1);
  const buckets = new Map<string, number[]>();
  const add = (key: string, index: number) => {
    const values = buckets.get(key) ?? [];
    if (!values.includes(index) && values.length < maxBucketSize) values.push(index);
    buckets.set(key, values);
  };
  for (let index = 0; index < metadata.length; index += 1) {
    const representative = dsu.find(index);
    const hash = metadata[representative].simhash;
    for (let band = 0; band < 4; band += 1) add(`s:${band}:${Number((hash >> BigInt(band * 16)) & 0xffffn)}`, representative);
    for (const token of metadata[representative].tokens.filter(token => (tokenFrequency.get(token) ?? 0) <= maxBucketSize).slice(0, 8)) add(`r:${token}`, representative);
  }
  const compared = new Set<string>(); let comparisons = 0;
  for (const values of buckets.values()) {
    for (let left = 0; left < values.length; left += 1) for (let right = left + 1; right < values.length; right += 1) {
      const a = values[left]; const b = values[right]; const key = `${a}:${b}`;
      if (compared.has(key)) continue; compared.add(key); comparisons += 1;
      if (comparisons > maxPairComparisons) throw new Error('AI event clustering comparison limit exceeded');
      if (!topicCompatible(sorted[a], sorted[b])) continue;
      ...
      if (jaccard >= minJaccard || (hamming <= maxHamming && jaccard >= 0.35)) dsu.union(a, b);
    }
  }
  const groups = new Map<number, SelectionCandidate[]>();
  for (let index = 0; index < sorted.length; index += 1) { const root = dsu.find(index); const values = groups.get(root) ?? []; values.push(sorted[index]); groups.set(root, values); }
  return [...groups.values()].map(buildEventCluster).sort((a, b) => a.clusterId.localeCompare(b.clusterId));
}

function buildEventCluster(input: SelectionCandidate[]): EventCluster {
  const members = [...input].sort((a, b) => a.record.storeId.localeCompare(b.record.storeId));
  if (members.length < 1) throw new Error('AI event cluster cannot be empty');
  ...
  const topics = [...new Set(members.flatMap(item => item.assessment.topics))].sort() as AIRelevanceTopic[];
...
export function clusterAIEventsFromGroups(candidates: SelectionCandidate[], storeIdGroups: string[][]): EventCluster[] {
  ...
  for (const candidate of candidates) {
    ...
    if (typeof storeId !== 'string' || !storeId || byId.has(storeId)) throw new Error('AI event candidates contain duplicate or invalid ids');
    byId.set(storeId, candidate);
  }
  const seen = new Set<string>();
  const clusters = storeIdGroups.map(group => {
    if (!Array.isArray(group) || group.length < 1) throw new Error('AI event grouping contains an empty cluster');
    const members = group.map(storeId => {
      if (typeof storeId !== 'string' || seen.has(storeId) || !byId.has(storeId)) throw new Error('AI event grouping contains a duplicate or unknown id');
      seen.add(storeId); return byId.get(storeId)!;
    });
    return buildEventCluster(members);
  });
  if (seen.size !== byId.size) throw new Error('AI event grouping omitted candidates');
  return clusters.sort((a, b) => a.clusterId.localeCompare(b.clusterId));
}

function clusterPriority(cluster: EventCluster): [number, number, number, number, number, string] {
...
function compareClusterPriority(left: EventCluster, right: EventCluster): number {
  const l = clusterPriority(left); const r = clusterPriority(right);
  for (let index = 0; index < 5; index += 1) { const delta = (r[index] as number) - (l[index] as number); if (delta !== 0) return delta; }
  return (l[5] as string).localeCompare(r[5] as string);
}

export function rankDiverseEvents(clusters: EventCluster[], options: RankingOptions): RankedEvent[] {
  if (!Number.isInteger(options.maxItems) || options.maxItems < 1 || options.maxItems > 100) throw new Error('maxItems is invalid');
  const maxPerSource = options.maxPerSource ?? Math.max(1, Math.ceil(options.maxItems * 0.2));
  if (!Number.isInteger(maxPerSource) || maxPerSource < 1 || maxPerSource > 100) throw new Error('maxPerSource is invalid');
  ...
  if (quota && (typeof quota.sourceId !== 'string' || !quota.sourceId.trim()
    ...
    || quota.maxItems > maxPerSource || quota.minItems > options.maxItems)) throw new Error('sourceQuota is invalid');
  ...
  const selected: EventCluster[] = []; const sourceCounts = new Map<string, number>(); const coveredTopics = new Set<string>();
  const canTake = (cluster: EventCluster) => {
  ...
  const take = (cluster: EventCluster) => { selected.push(cluster); const source = cluster.representative.record.sourceId; sourceCounts.set(source, (sourceCounts.get(source) ?? 0) + 1); for (const topic of cluster.topics) coveredTopics.add(topic); };
  if (quota) {
    const quotaCandidates = remaining.filter(cluster => cluster.representative.record.sourceId === quota.sourceId);
    if (quotaCandidates.length < quota.minItems) throw new Error(`Selection source quota cannot be satisfied for ${quota.sourceId}: requires at least ${quota.minItems}, found ${quotaCandidates.length}`);
    for (const cluster of quotaCandidates.slice(0, quota.minItems)) take(cluster);
  }
  for (const cluster of remaining) {
    if (selected.length >= options.maxItems) break;
    if (canTake(cluster) && cluster.topics.some(topic => !coveredTopics.has(topic))) take(cluster);
  }
  ...
  for (const cluster of longTail) {
    if (selected.length >= options.maxItems || selected.filter(item => item.signals.distinctSourceCount === 1).length >= longTailTarget) break;
    if (canTake(cluster)) take(cluster);
  }
  for (const cluster of remaining) {
    if (selected.length >= options.maxItems) break;
    if (canTake(cluster)) take(cluster);
  ...
  return selected.map((cluster, index) => ({
    ...
    reasons: [quota && cluster.representative.record.sourceId === quota.sourceId ? 'source-quota' : undefined,
      ...
      cluster.signals.topicCount > 1 ? 'multi-topic-event' : 'topic-representative'].filter((reason): reason is string => Boolean(reason)),
  }));
}

export function estimateMaterialTokens(value: string): number {
  let tokens = 0; let latinRun = 0;
  const flush = () => { if (latinRun > 0) { tokens += Math.ceil(latinRun / 4); latinRun = 0; } };
  for (const char of value) {
    if (/[\u3400-\u9fff]/u.test(char)) { flush(); tokens += 1; }
    else if (/[A-Za-z0-9]/u.test(char)) latinRun += 1;
    else { flush(); if (!/\s/u.test(char)) tokens += 1; }
  }
  flush(); return tokens + 32;
}
...
function directMediaKind(value: string): MaterialMedia['kind'] | undefined {
  try {
    const pathname = new URL(value).pathname;
    if (DIRECT_IMAGE_EXTENSION.test(pathname)) return 'image';
    if (DIRECT_VIDEO_EXTENSION.test(pathname)) return 'video';
  } catch { /* rejected by mediaUrl */ }
  return undefined;
}

interface MediaHtmlToken {
  start: number;
  end: number;
  name: string;
  closing: boolean;
...
function stripSelectionMarkdownCode(value: string): string {
  ...
  const blockSafe = lines.map(line => {
    ...
    if (/^(?: {4}|\t)/u.test(text)) return line.endsWith('\n') ? '\n' : '';
    return line;
  }).join('');

  let result = '';
  for (let index = 0; index < blockSafe.length;) {
    if (blockSafe[index] !== '`' || escapedAt(blockSafe, index)) { result += blockSafe[index]; index += 1; continue; }
    let runEnd = index;
    while (blockSafe[runEnd] === '`') runEnd += 1;
    const length = runEnd - index;
    let cursor = runEnd; let closing = -1;
    while (cursor < blockSafe.length) {
      const found = blockSafe.indexOf('`', cursor);
      if (found < 0) break;
      let end = found;
      while (blockSafe[end] === '`') end += 1;
      if (!escapedAt(blockSafe, found) && end - found === length) { closing = end; break; }
      cursor = end;
    }
    if (closing < 0) { result += blockSafe.slice(index, runEnd); index = runEnd; continue; }
...
function selectionHtmlTokens(value: string): MediaHtmlToken[] {
  const tokens: MediaHtmlToken[] = [];
  for (let start = value.indexOf('<'); start >= 0; start = value.indexOf('<', start + 1)) {
    let quote: string | undefined; let end = start + 1;
    ...
    if (!header) continue;
    const attributes = new Map<string, string>(); let cursor = header[0].length;
    while (cursor < raw.length - 1 && !header[1]) {
      while (/\s/u.test(raw[cursor] ?? '')) cursor += 1;
      if (raw[cursor] === '/' || raw[cursor] === '>') break;
      ...
      if (raw[cursor] === '=') {
        ...
        if (delimiter) while (cursor < raw.length && raw[cursor] !== delimiter) cursor += 1;
        else while (cursor < raw.length && !/[\s>]/u.test(raw[cursor])) cursor += 1;
        attributeValue = raw.slice(valueStart, cursor);
        if (delimiter && raw[cursor] === delimiter) cursor += 1;
      }
...
type MaskFrame = { name: string; mode: 'erase'; start: number } | { name: string; mode: 'video'; start: number; bodyStart: number };

function hiddenHtmlToken(token: MediaHtmlToken): boolean {
  if (token.attributes.has('hidden')) return true;
  if (token.attributes.get('aria-hidden')?.trim().toLocaleLowerCase('en-US') === 'true') return true;
...
function stripSelectionHtmlNonRendered(value: string): string {
  ...
  const stack: MaskFrame[] = []; const ranges: Array<[number, number]> = [];
  for (const token of selectionHtmlTokens(safe)) {
    ...
    const mode: MaskFrame['mode'] | undefined = hiddenHtmlToken(token) || excluded.has(token.name) ? 'erase'
    ...
    if (isVoid) {
      if (mode === 'erase') ranges.push([token.start, token.end]);
      continue;
    }
    stack.push(mode === 'video'
      ? { name: token.name, mode, start: token.start, bodyStart: token.end }
      : { name: token.name, mode, start: token.start });
    if (token.name === 'plaintext') break;
  }
  for (const item of stack) ranges.push(item.mode === 'video' ? [item.bodyStart, safe.length] : [item.start, safe.length]);
 ...
 */
export function extractSelectionMedia(description: unknown, content: unknown, maxMediaPerItem = 16, contentHtml?: unknown): MaterialMedia[] {
  if (!Number.isInteger(maxMediaPerItem) || maxMediaPerItem < 0 || maxMediaPerItem > 64) throw new Error('maxMediaPerItem is invalid');
  const images: MaterialMedia[] = []; const videos: MaterialMedia[] = []; const seen = new Set<string>();
  const add = (raw: string, kind?: MaterialMedia['kind']) => {
    const url = mediaUrl(raw.trim().replace(/[.,;:!?，。；：！？]+$/u, ''));
    const resolvedKind = kind ?? (url ? directMediaKind(url) : undefined);
    if (!url || !resolvedKind || seen.has(url)) return;
    seen.add(url);
    const target = resolvedKind === 'video' ? videos : images;
    if (target.length < 64) target.push({ kind: resolvedKind, url });
  };
  for (const body of [description, content, contentHtml]) {
    ...
    for (const match of markdownSafe.matchAll(/!\[[^\]]*\]\(\s*<?(https?:\/\/[^\s)>]+)>?(?:\s+["'][^)]*["'])?\s*\)/giu)) {
      if (!escapedAt(markdownSafe, match.index)) add(match[1], 'image');
    }
    const videos: Array<{ media: MaterialMedia[] }> = [];
    for (const token of tokens) {
      if (escapedAt(safe, token.start)) continue;
      ...
      if (token.name === 'video' && !token.closing) {
        videos.push({ media: [
          token.attributes.get('src') ? { url: token.attributes.get('src')!, kind: 'video' as const } : undefined,
          token.attributes.get('poster') ? { url: token.attributes.get('poster')!, kind: 'image' as const } : undefined,
        ].filter((item): item is MaterialMedia => item !== undefined) });
        continue;
      }
      ...
      if (token.name === 'video' && token.closing && videos.length > 0) {
        ...
        for (const media of frame.media) add(media.url, media.kind);
      }
    }
    ...
    const directSafe = markdownSafe.replace(
      /!?\[[^\]]*\]\(\s*<?https?:\/\/[^\s)>]+>?(?:\s+["'][^)]*["'])?\s*\)/giu,
    ...
    for (const match of directSafe.matchAll(/https?:\/\/[^\s<>"'()\[\]]+/giu)) add(match[0]);
  }
...
function excerpt(field: 'description' | 'content', source: string, start: number, end: number): MaterialExcerpt | undefined {
  const safeStart = Math.max(0, Math.min(start, source.length)); const safeEnd = Math.max(safeStart, Math.min(end, source.length));
...
export function buildPackedMaterial(candidate: SelectionCandidate, maxChars: number, maxMediaPerItem = 16): PackedMaterial {
  if (!Number.isInteger(maxChars) || maxChars < 256 || maxChars > 20_000) throw new Error('Packed material maxChars is invalid');
  if (!Number.isInteger(maxMediaPerItem) || maxMediaPerItem < 0 || maxMediaPerItem > 64) throw new Error('maxMediaPerItem is invalid');
  const item = candidate.record.item as Record<string, unknown>;
  const description = typeof item.description === 'string' ? item.description : '';
  const content = typeof item.content === 'string' ? item.content : '';
  const metadata = item.metadata && typeof item.metadata === 'object' && !Array.isArray(item.metadata)
    ? item.metadata as Record<string, unknown> : {};
  const contentHtml = typeof metadata.content_html === 'string' ? metadata.content_html : '';
  const sourceField: 'description' | 'content' = description.length >= content.length ? 'description' : 'content';
  ...
  const candidates: Array<MaterialExcerpt | undefined> = [
  ...
  for (const evidence of candidate.assessment.evidence) {
    if (!evidence.phrase) continue;
    const index = normalizedBody.indexOf(evidence.phrase.toLocaleLowerCase('en-US'));
    if (index >= 0) candidates.push(excerpt(sourceField, body, index - Math.floor(segment / 2), index + evidence.phrase.length + Math.floor(segment / 2)));
  }
  const seen = new Set<string>(); const excerpts: MaterialExcerpt[] = [];
  for (const value of candidates) { if (!value || seen.has(value.sha256)) continue; seen.add(value.sha256); excerpts.push(value); }
  const base = {
    ...
    author: clean((item as Record<string, unknown>).author, Math.min(512, Math.max(0, Math.floor(maxChars * 0.06)))),
  ...
  while (excerpts.length > 0 && JSON.stringify({ ...base, excerpts, media }).length > maxChars) excerpts.pop();
  let encoded = JSON.stringify({ ...base, excerpts, media });
  if (encoded.length > maxChars) {
    base.url = ''; base.author = ''; base.category = ''; base.source = '';
    base.title = base.title.slice(0, Math.max(0, maxChars - 256));
    encoded = JSON.stringify({ ...base, excerpts, media });
  }
  if (encoded.length > maxChars) throw new Error('Packed material metadata exceeds per-item budget');
...
export function packSelectionMaterials(ranked: RankedEvent[], options: PackingOptions): Array<{ ranked: RankedEvent; material: PackedMaterial }> {
  const minChars = options.minCharsPerItem ?? 600; const maxChars = options.maxCharsPerItem ?? 3_000;
  ...
  if (!Number.isInteger(options.maxMaterialChars) || options.maxMaterialChars < 4_096 || options.maxMaterialChars > 1_000_000
    ...
    || !Number.isInteger(maxMediaPerItem) || maxMediaPerItem < 0 || maxMediaPerItem > 64) throw new Error('Material packing limits are invalid');
  const result: Array<{ ranked: RankedEvent; material: PackedMaterial }> = [];
  let chars = 2; let tokens = 0;
  for (const item of ranked) {
    ...
    let material: PackedMaterial;
    try { material = buildPackedMaterial(item.cluster.representative, allocation, maxMediaPerItem); } catch { continue; }
    const separator = result.length > 0 ? 1 : 0;
    const serializedChars = JSON.stringify(material).length;
    if (chars + separator + serializedChars > options.maxMaterialChars || tokens + material.estimatedTokens > options.maxMaterialTokens) continue;
    chars += separator + serializedChars; tokens += material.estimatedTokens; result.push({ ranked: item, material });
  }
  return result;
}
...
function canonical(value: unknown): string {
  ...
  if (value && typeof value === 'object') { const source = value as Record<string, unknown>; return `{${Object.keys(source).sort().map(key => `${JSON.stringify(key)}:${canonical(source[key])}`).join(',')}}`; }
  return JSON.stringify(value);
}

export function selectionSha256(value: unknown): string {
