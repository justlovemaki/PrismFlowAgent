...
import type { ProductionArtifact, ProductionArtifactV2, ProductionDestinationPresentation, WechatCropRatio } from '../production/ContentProduction.js';

export type WechatArticleType = 'news' | 'newspic';

export interface WechatPublisherProfile {
  articleType: WechatArticleType;
  defaultAuthor: string;
  digestPolicy: 'omit' | 'plain-text-excerpt' | 'artifact-or-omit' | 'artifact-or-plain-text-excerpt';
  needOpenComment: 0 | 1;
...
export interface WechatMarkdownImage {
  source: string;
  assetId?: string;
  alt: string;
  placeholder: string;
...
export interface RenderedWechatMarkdown {
  html: string;
  plainText: string;
  newspicText: string;
  images: WechatMarkdownImage[];
}
...
function normalizeAssetSource(source: string): string | undefined {
  const match = /^(?:prismflow-media:(?:\/\/)?)([a-f0-9]{64})$/u.exec(source);
  return match?.[1];
}

function safeWebUrl(value: string, label: string, allowHttp = false): string {
  let parsed: URL;
...
function inlineMarkdown(value: string, images: WechatMarkdownImage[]): string {
  const stash: string[] = [];
  const hold = (html: string) => { const token = `\u0000${stash.length}\u0000`; stash.push(html); return token; };
  let text = value.replace(/`([^`\n]+)`/gu, (_all, code: string) => hold(`<code style="font-family:monospace;background:#f6f8fa;padding:0 3px;">${escapeHtml(code)}</code>`));
  text = text.replace(/!\[([^\]\n]*)\]\(([^\s)]+)(?:\s+"[^"]*")?\)/gu, (_all, alt: string, source: string) => {
    const normalizedSource = decodeEntities(source);
    const assetId = normalizeAssetSource(normalizedSource);
    if (!assetId) safeWebUrl(normalizedSource, 'Markdown image', true);
  ...
  text = text.replace(/\[([^\]\n]+)\]\(([^\s)]+)(?:\s+"[^"]*")?\)/gu, (_all, label: string, target: string) => {
  ...
  text = escapeHtml(text)
    ...
    .replace(/(?<!\*)\*([^*\n]+)\*(?!\*)/gu, '<em>$1</em>');
  return text.replace(/\u0000(\d+)\u0000/gu, (_all, index: string) => stash[Number(index)] ?? '');
}

function decodeEntities(value: string): string {
  const named: Record<string, string> = { amp: '&', lt: '<', gt: '>', quot: '"', apos: "'", nbsp: ' ', copy: '©', reg: '®', hellip: '…', ndash: '–', mdash: '—', middot: '·' };
  return value.replace(/&(?:#(\d+)|#x([a-f0-9]+)|([a-z]+));/giu, (entity, decimal: string, hexadecimal: string, name: string) => {
...
export function renderWechatMarkdown(markdown: string): RenderedWechatMarkdown {
  if (typeof markdown !== 'string' || markdown.length < 1 || /[\u0000\u007f]/u.test(markdown)) throw new Error('Markdown is invalid');
  const lines = markdown.replaceAll('\r\n', '\n').replaceAll('\r', '\n').split('\n');
  const html: string[] = [`<section style="${HTML_STYLE}">`];
  const plain: string[] = [];
  const newspic: string[] = [];
  const images: WechatMarkdownImage[] = [];
  ...
  const flushParagraph = () => {
    ...
    if (text) { plain.push(text); newspic.push(text); }
    paragraph = [];
  };
  ...
  const flushTable = () => {
    ...
    tableRows.forEach((row, rowIndex) => {
      html.push('<tr>'); row.forEach(cell => html.push(`<${rowIndex === 0 ? 'th' : 'td'} style="${rowIndex === 0 ? WECHAT_STYLE.th : WECHAT_STYLE.td}">${inlineMarkdown(cell, images)}</${rowIndex === 0 ? 'th' : 'td'}>`)); html.push('</tr>');
      const cells = row.map(cell => plainInline(cell).trim()).filter(Boolean);
      const text = cells.join(' '); if (text) plain.push(text);
      const newspicRow = cells.join(' | '); if (newspicRow) newspic.push(newspicRow);
    });
    html.push('</table>'); tableRows = [];
  };
  const flushFlow = () => { flushParagraph(); closeList(); flushTable(); };
  const headingHtml = (depth: number, text: string) => depth === 1
  ...
  for (const line of lines) {
    if (code) {
      if (/^\s*```/u.test(line)) {
        ...
        if (code.length) { const text = code.join('\n'); plain.push(text); newspic.push(text); } code = undefined;
      } else code.push(line);
      continue;
    }
    if (/^\s*```/u.test(line)) { flushFlow(); code = []; continue; }
    const trimmed = line.trim();
    if (!trimmed) { flushFlow(); newspic.push(''); continue; }
    if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
      flushParagraph(); closeList();
      const cells = trimmed.slice(1, -1).split('|').map(cell => cell.trim());
      if (!cells.every(cell => /^:?-{3,}:?$/u.test(cell))) tableRows.push(cells);
      continue;
    }
    flushTable();
    const heading = /^(#{1,6})\s+(.+)$/u.exec(line);
    if (heading) {
      flushParagraph(); closeList(); html.push(headingHtml(heading[1].length, heading[2]));
      { const text = plainInline(heading[2]).trim(); plain.push(text); newspic.push(text); } continue;
    }
    if (trimmed === '---' || trimmed === '***') {
      flushParagraph(); closeList(); html.push('<hr style="border:0;height:1px;background:#eee;margin:40px 0;"/>'); continue;
    }
    const item = /^\s*(?:(\d+)\.|[-+*])\s+(.+)$/u.exec(line);
    if (item) {
      flushParagraph(); const next = item[1] ? 'ol' : 'ul'; if (list && list !== next) closeList();
      if (!list) { list = next; html.push(`<${list} style="${WECHAT_STYLE.list}">`); }
      html.push(`<li style="${WECHAT_STYLE.item}">${inlineMarkdown(item[2], images)}</li>`);
      { const text = plainInline(item[2]).trim(); plain.push(text); newspic.push(item[1] ? `${item[1]}. ${text}` : `• ${text}`); } continue;
    }
    const quote = /^\s*>\s?(.*)$/u.exec(line);
    if (quote) {
      flushParagraph(); closeList(); html.push(`<blockquote style="${WECHAT_STYLE.quote}">${inlineMarkdown(quote[1], images)}</blockquote>`);
      { const text = plainInline(quote[1]).trim(); plain.push(text); newspic.push(`> ${text}`); } continue;
    }
    paragraph.push(line);
  }
  if (code) throw new Error('Markdown code fence is not closed');
  flushFlow(); html.push('</section>');
  return {
    html: html.join(''),
...
export function destinationPresentation(artifact: ProductionArtifact, publisherId: string): ProductionDestinationPresentation | undefined {
  return (artifact as ProductionArtifactV2).destinationPresentations?.find(item => item.publisherId === publisherId);
}

export function validateWechatCrops(articleType: WechatArticleType, presentation?: ProductionDestinationPresentation): void {
  const allowed = new Set<WechatCropRatio>(articleType === 'news' ? ['2.35_1', '1_1'] : ['2.35_1', '1_1', '16_9']);
  for (const crop of presentation?.cover?.crops ?? []) if (!allowed.has(crop.ratio)) throw new Error(`Crop ratio ${crop.ratio} is not allowed for ${articleType}`);
}

export function resolveWechatText(profile: WechatPublisherProfile, artifact: ProductionArtifact, publisherId: string, rendered: RenderedWechatMarkdown) {
  ...
  if (presentation?.digest !== undefined && profile.digestPolicy.startsWith('artifact-or-')) digest = presentation.digest;
  ...
  for (const [field, value, max] of [['title', artifact.title, profile.limits.titleChars], ['author', author, profile.limits.authorChars], ['digest', digest, profile.limits.digestChars]] as const) {
    if (value !== undefined && codePointLength(value) > max) throw new Error(`WeChat ${field} exceeds its configured limit`);
  }
  return { presentation, author, digest };
}

export function validateWechatContent(profile: WechatPublisherProfile, content: string): void {
...
export function renderNewspicContent(artifact: ProductionArtifact, digest: string | undefined, rendered: RenderedWechatMarkdown): string {
  const plain = rendered.newspicText || [artifact.title, digest].filter((value): value is string => typeof value === 'string' && value.length > 0).join('\n');
...
export function replaceWechatImageUrls(rendered: RenderedWechatMarkdown, urls: string[]): string {
  if (urls.length !== rendered.images.length) throw new Error('WeChat body image result count is invalid');
  return rendered.images.reduce((html, image, index) => {
    const marker = `src="${image.placeholder}"`;
    if (html.split(marker).length !== 2) throw new Error('WeChat body image placeholder is missing or duplicated');
    return html.replace(marker, `src="${escapeHtml(safeWebUrl(urls[index], 'WeChat uploaded image'))}"`);
  }, rendered.html);
}

export function orderedNewspicAssetIds(rendered: RenderedWechatMarkdown, presentation: ProductionDestinationPresentation | undefined, maxImages: number): string[] {
  ...
  if (ordered.length < 1 || ordered.length > Math.min(20, maxImages)) throw new Error('newspic requires from 1 to 20 approved images');
  if (ordered.some(assetId => !ASSET_ID.test(assetId))) throw new Error('newspic image asset id is invalid');
  return ordered;
}
