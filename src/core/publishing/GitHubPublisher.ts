...
export function parseGitHubRepository(value: string): GitHubRepository {
  ...
  if (parts.length !== 2) throw new Error('GitHub repository must use owner/repo format');
...
export function validateGitHubBranch(value: string): string {
...
export function validateGitHubPathPrefix(value: string): string {
  if (value === '') return '';
...
export function buildGitHubPublicationPath(pathPrefix: string, fileName: string): string {
...
export function renderGitHubCommitMessage(pattern: string, date: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error(`Invalid GitHub publication date: ${date}`);
...
export function buildGitHubContentsApiUrl(
  ...
  const url = new URL(
    `${base}/repos/${encodeURIComponent(repository.owner)}/${encodeURIComponent(repository.repo)}/contents/${encodedPath}`,
  );
  if (branch !== undefined) url.searchParams.set('ref', validateGitHubBranch(branch));
  return url.toString();
}
